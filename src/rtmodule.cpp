...
Radiation::Radiation(const RadiationParameters& rp) {
	K1 = rp.K1;
	K2 = rp.K2;
	K3 = rp.K3;
	K4 = rp.K4;
...
int Radiation::getRayPlane(GridCell* cptr, GridCell* srcptr, Grid3D* gptr) const {
...
void Radiation::updateTauSC(bool average, GridCell* cptr, GridCell* srcptr, Grid3D* gptr) const {
	if(cptr != srcptr){
		int plane = getRayPlane(cptr, srcptr, gptr);
		int icoord = (plane+1)%3, jcoord = (plane+2)%3, kcoord = (plane%3);
		int cpos[3] = {cptr->xc[icoord], cptr->xc[jcoord], cptr->xc[kcoord]};
		int spos[3] = {srcptr->xc[icoord], srcptr->xc[jcoord], srcptr->xc[kcoord]};
...
double Radiation::cellPathLength(GridCell* cptr, GridCell* srcptr, Grid3D* gptr) const {
	double ds = 0.0, denom;
	double dx[3] = {gptr->dx[0], gptr->dx[1], gptr->dx[2]};
	int dxcs[3];
	for(int i = 0; i < 3; i++)
		dxcs[i] = cptr->xc[i]-srcptr->xc[i];
...
double Radiation::shellVolume(GridCell* cptr, GridCell* srcptr, Grid3D* gptr) const {
	double r_sqrd = 0;
	double ds = cellPathLength(cptr, srcptr, gptr);
	double dx[3] = {gptr->dx[0], gptr->dx[1], gptr->dx[2]};
	for(int i=0; i<3; i++)
		r_sqrd += (cptr->xc[i]-srcptr->xc[i])*(cptr->xc[i]-srcptr->xc[i])*dx[i]*dx[i];
...
double Radiation::PIrate(double frac, double T, double delT, GridCell* cptr, GridCell* srcptr, Grid3D* gptr) const {
	double vol = shellVolume(cptr, srcptr, gptr); 
...
void Radiation::update_dtau(GridCell* cptr, GridCell* srcptr, Grid3D* gptr) const {
	if(cptr != srcptr){
		double ds = cellPathLength(cptr, srcptr, gptr);
...
void Radiation::update_HIIfrac(double dt, GridCell* cptr, GridCell* srcptr, Grid3D* gptr) const {
	int scheme = 0;
	if(cptr != srcptr){
		if(scheme == 0){
			...
			ds = cellPathLength(cptr, srcptr, gptr);
			...
			while(!converged){
				...
				A_pi = PIrate(HII_avg, tau_avg, dtau_avg, cptr, srcptr, gptr);
			...
			tau = cptr->R[itau];
			dtau = cptr->R[idtau];
			A_pi = PIrate(HII, tau, dtau, cptr, srcptr, gptr);
...
double Radiation::getTimeStep(double dt_dyn, GridCell* srcptr, Grid3D* gptr) const {
	...
	for(GridCell* cptr = srcptr; cptr != NULL; cptr = gptr->nextCausal(cptr, srcptr)){
		...
		dt3 = dt_dyn;
		dt4 = dt_dyn;
		double A_pi = PIrate(cptr->Q[ihii], cptr->R[itau], cptr->R[idtau], cptr, srcptr, gptr);
...
void Radiation::transferRadiation(double dt, double& IF, Grid3D* gptr) const {
	...
	for(GridCell* cptr = srcptr; cptr != NULL; cptr = gptr->nextCausal(cptr, srcptr)){
		updateTauSC(average==false, cptr, srcptr, gptr);
		updateTauSC(average==true, cptr, srcptr, gptr);
		update_HIIfrac(dt, cptr, srcptr, gptr); // updates HII fraction (see gridcell.cpp).
		update_dtau(cptr, srcptr, gptr);
