...
int StreambufGZ::underflow() { // used for input buffer only
	if ( gptr() && ( gptr() < egptr()))
...
StreamGZ_Base::StreamGZ_Base( const char* name, int mode) {
	init( &buf);
	open( name, mode);
}

StreamGZ_Base::~StreamGZ_Base() {
	buf.close();
}

void StreamGZ_Base::open( const char* name, int open_mode) {
	if ( ! buf.open( name, open_mode))
		clear( rdstate() | std::ios::badbit);
}

void StreamGZ_Base::close() {
	if ( buf.isOpen())
		if ( ! buf.close())
			clear( rdstate() | std::ios::badbit);
}
