...
def addImage(index, grid, xrange):
	...
	filename = mp1_data.getRadioDirname(col, row, 25, 45, 5) + "/emeasure_ff.fits"
	...
	if col == 0:
		text_str = fmt.latexify("n_\\star = " + fmt.fmt_power(mp1_data.densities[2 * row], '{:3.1f}', 4) + "\ \\mathrm{cm^{-3}}")
...
hrats = [1, 1, 1, 1, 1]
vrats = [1, 1, 1]
fancy_grid = torch.FancyAxesGrid(hrats, vrats, border=(0.06, 0.01, 0.05, 0.02),
								 fig_width=plot_size, dpi=DPI, fig_format=figformat,
								 csize=0.02, cspace=0.008, cpad=0.035,
								 hspace=0.035, vspace=0.07)

xrange = []
xrange.append([15, 62, 134, 260, 800])
xrange.append([6, 34, 90, 150, 400])
xrange.append([2.52, 16, 54, 100, 190])
...
fancy_grid.save_plot("emeasure-mvd-combined.png")