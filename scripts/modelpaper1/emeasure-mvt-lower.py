...
def addImage(col, row, grid, xrange):
	...
	filename = mp1_data.getRadioDirname(imass, 2, snapshots[itime], 45, 5) + "/emeasure_ff.fits"
	...
	param_filename = mp1_data.getParamFilename(imass, 2, snapshots[itime])
	...
	if False:
		...
		intensity_filename = mp1_data.getRadioDirname(imass, 2, snapshots[itime], 45, 5) + "/intensity_pixel_ff.fits"
	...
	if col == 0:
		ax.text(-0.22, 0.5,fmt.latexify("M = " + fmt.fmt_mass(mp1_data.masses[row + 5]) + "\ \\mathrm{M_\\odot}"),
...
xrange = []
xrange.append([0.52, 0.6, 0.9, 1.1])
