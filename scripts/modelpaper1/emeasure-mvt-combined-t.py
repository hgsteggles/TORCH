...
def addImage(col, row, grid, xrange):
	...
	filename = mp1_data.getRadioDirname(col, 2, snapshots[row], 45, 5) + "/emeasure_ff.fits"
	...
	ax.set_xlim([-xrange[row][col]/2.0, xrange[row][col]/2.0])
	ax.set_ylim([-xrange[row][col]/2.0, xrange[row][col]/2.0])
	ax.xaxis.set_ticks(np.arange(-xrange[row][col]/2.0, (xrange[row][col]/2.0) + 0.0001, xrange[row][col]/4.0))
	ax.yaxis.set_ticks(np.arange(-(xrange[row][col]/2.0), (xrange[row][col]/2.0) + 0.0001, xrange[row][col]/4.0))
	...
	if False:
		...
		ax.plot([(0.90)*xrange[row][col]/2.0 - FWHM, (0.90)*xrange[row][col]/2.0], [-(0.90)*xrange[row][col]/2.0, -(0.90)*xrange[row][col]/2.0])
	...
	if col == 0:
		text_str = fmt.latexify("t = " + fmt.fmt_power(int(mp1_data.times[2 * row] / 1000.0), '{:3.0f}', 0) + "\ \\mathrm{kyr}")
		ax.text(-0.26, 0.5, text_str,
				fontsize=8, horizontalalignment='right', verticalalignment='center',
				rotation='vertical', transform=ax.transAxes)
	if row == 0:
		text_str = fmt.latexify("M = " + fmt.fmt_mass(mp1_data.masses[2 * col]) + "\ \\mathrm{M_\\odot}")
		ax.text(0.5, 1.34, text_str,
...
hrats = [1, 1, 1, 1, 1]
vrats = [1, 1, 1]
fancy_grid = torch.FancyAxesGrid(hrats, vrats, border=(0.06, 0.01, 0.06, 0.02),
								 fig_width=plot_size, dpi=DPI, fig_format=figformat,
								 csize=0.02, cspace=0.008, cpad=0.035,
								 hspace=0.035, vspace=0.07)


xrange = []
xrange.append([0.040, 0.18, 0.40, 0.6, 1.1])
xrange.append([0.038, 0.24, 0.65, 1.0, 3.0])
xrange.append([0.040, 0.25, 0.80, 1.6, 3.6])
...
for i in range(5):
	for j in range(9):
		addImage(j, i, fancy_grid, xrange)

fancy_grid.update_cbar()

fancy_grid.save_plot("emeasure-mvt-combined.png")