...
def addImage(index, grid, xrange):
	...
	if row == 0:
		text_str = fmt.latexify("n_\\star = " + fmt.fmt_power(mp1_data.densities[2 * col], '{:3.1f}', 4) + "\ \\mathrm{cm^{-3}}")
		ax.text(0.5, 1.30, text_str,
...
hrats = [1, 1, 1]
vrats = [1, 1, 1, 1, 1]
fancy_grid = torch.FancyAxesGrid(hrats, vrats, border=(0.1, 0.01, 0.03, 0.012),
								 fig_width=plot_size, dpi=DPI, fig_format=figformat,
								 csize=0.010, cspace=0.005, cpad=0.055,
								 hspace=0.05, vspace=0.035)
