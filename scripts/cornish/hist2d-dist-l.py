...
if args.harry:
	benstr = "-harry"
outputfile = cornish_data.dirname + '/hist2d-dist-l' + benstr + '.' + figformat
...
grid[0].set_ylabel(plotter.format_label(torch.VarType('l', units='deg')))
grid[1].set_ylabel(plotter.format_label(torch.VarType('l', units='deg')))

grid[0].set_xlim([0, 20])
grid[1].set_xlim([0, 20])

grid[0].set_ylim([10, 65])
grid[1].set_ylim([10, 65])

xbins = np.arange(0, 20, 1)
ybins = np.arange(10, 65, 2)

counts_0, xedges_0, yedges_0 = np.histogram2d(simulated_survey[:,4], simulated_survey[:,11], bins=(xbins, ybins))
counts_1, xedges_1, yedges_1 = np.histogram2d(cornish_survey[:,33], cornish_survey[:,1], bins=(xbins, ybins))
...
for i in range(len(xedges_0) - 1):
	x_0.append(0.5 * (xedges_0[i + 1] + xedges_0[i]))
y_0 = []
