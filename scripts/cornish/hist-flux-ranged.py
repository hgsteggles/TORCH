...
if args.harry:
	benstr = "-harry"
outputfile = cornish_data.dirname + '/hist-flux-ranged' + benstr + '.' + figformat
...
selected = []
for i in range(len(simulated_survey[:,9])):
	dist = simulated_survey[i,4]
	if dist > 4 and dist < 6:
		selected.append(simulated_survey[i,9])

kx1 = dict(linewidth=1.5, label="CORNISH")
kx2 = dict(linewidth=1.5, label="Simulated", linestyle='--')
plotter.histstep(grid[0], cornish_survey[:,9], np.logspace(0.3, 4.3, 22), **kx1)
plotter.histstep(grid[0], selected, np.logspace(0.3, 4.3, 22), **kx2)
