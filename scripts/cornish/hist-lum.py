...
if args.harry:
	benstr = "-harry"
outputfile = cornish_data.dirname + '/hist-lum' + benstr + '.' + figformat
...
for i in range(len(simulated_survey[:,0])):
	luminosities.append(np.power(10.0, logL_interp(simulated_survey[i,1])))
...
grid[0].set_xlabel(plotter.format_label(torch.VarType('L', units='L_\\odot')))
grid[0].set_ylabel(plotter.format_label(torch.VarType('N')))
grid[0].set_xlim([1.0e2, 1.0e6])
