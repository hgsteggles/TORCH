...
if args.harry:
	benstr = "-harry"
outputfile = cornish_data.dirname + '/hist2d-dist-b' + benstr + '.' + figformat
...
grid[0].set_ylabel(plotter.format_label(torch.VarType('b', units='deg')))
grid[1].set_ylabel(plotter.format_label(torch.VarType('b', units='deg')))
...
ybins = np.arange(-1, 1, 0.1)

counts_0, xedges_0, yedges_0 = np.histogram2d(simulated_survey[:,4], simulated_survey[:,12], bins=(xbins, ybins))
counts_1, xedges_1, yedges_1 = np.histogram2d(cornish_survey[:,33], cornish_survey[:,2], bins=(xbins, ybins))
