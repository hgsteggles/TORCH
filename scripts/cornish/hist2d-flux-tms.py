...
if args.harry:
	benstr = "-harry"
outputfile = cornish_data.dirname + '/hist2d-flux-tms' + benstr + '.' + figformat
...
grid[0].set_ylabel(plotter.format_label(torch.VarType('t_\mathrm{ms}', units='kyr')))

grid[0].set_xlim([2, 10000])

grid[0].set_ylim([0.0, 200])

xbins = np.logspace(0.3, 4.3, 22)
ybins = np.arange(0, 200, 5)

counts_0, xedges_0, yedges_0 = np.histogram2d(simulated_survey[:,9], simulated_survey[:,2], bins=(xbins, ybins))
...
grid[0].hist2d(simulated_survey[:,9], simulated_survey[:,2], bins=(xbins, ybins))
