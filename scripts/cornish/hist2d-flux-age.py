...
if args.harry:
	benstr = "-harry"
outputfile = cornish_data.dirname + '/hist2d-flux-age' + benstr + '.' + figformat
...
grid[0].set_ylabel(plotter.format_label(torch.VarType('\mathrm{Age}', units='kyr')))
...
counts_0, xedges_0, yedges_0 = np.histogram2d(simulated_survey[:,9], simulated_survey[:,3], bins=(xbins, ybins))

counts_0 = ndimage.gaussian_filter(counts_0, sigma=(1, 1), order=0)

print np.max(counts_0)
