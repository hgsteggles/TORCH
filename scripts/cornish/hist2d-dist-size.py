...
if args.harry:
	benstr = "-harry"
outputfile = cornish_data.dirname + '/hist2d-dist-size' + benstr + '.' + figformat
...
grid[0].set_xlabel(plotter.format_label(torch.VarType('d', units='kpc')))
grid[1].set_xlabel(plotter.format_label(torch.VarType('d', units='kpc')))

grid[0].set_ylabel(plotter.format_label(torch.VarType('\mathrm{Angular\ Size}', units='asec')))
grid[1].set_ylabel(plotter.format_label(torch.VarType('\mathrm{Angular\ Size}', units='asec')))

grid[0].set_xlim([0, 20])
grid[1].set_xlim([0, 20])

grid[0].set_ylim([0.0, 25.0])
grid[1].set_ylim([0.0, 25.0])

xbins = np.arange(0, 20, 1)
ybins = np.arange(0, 25, 1)

counts_0, xedges_0, yedges_0 = np.histogram2d(simulated_survey[:,4], simulated_survey[:,7], bins=(xbins, ybins))
counts_1, xedges_1, yedges_1 = np.histogram2d(cornish_survey[:, 33], cornish_survey[:,11], bins=(xbins, ybins))
...
grid[0].hist2d(simulated_survey[:,4], simulated_survey[:,7], bins=(xbins, ybins))
grid[0].pcolormesh(xedges_0, yedges_0, np.swapaxes(counts_0, 0, 1))

grid[1].hist2d(cornish_survey[:,33], cornish_survey[:,11], bins=(xbins, ybins))
grid[1].pcolormesh(xedges_0, yedges_0, np.swapaxes(counts_1, 0, 1))

ts = 0.04
grid[0].text(0.5, 1-ts, "Simulated", fontsize=2*fontsize,
			 color='black', horizontalalignment='center',
			 verticalalignment='top', transform = grid[0].transAxes)
grid[1].text(0.5, 1-ts, "CORNISH", fontsize=2*fontsize,
			 color='black', horizontalalignment='center',
			 verticalalignment='top', transform = grid[1].transAxes)
