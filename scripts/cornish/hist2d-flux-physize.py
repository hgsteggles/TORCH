...
if args.harry:
	benstr = "-harry"
outputfile = cornish_data.dirname + '/hist2d-flux-physize' + benstr + '.' + figformat
...
grid[0].set_ylabel(plotter.format_label(torch.VarType('\mathrm{Physical\ Size}', units='asec')))
grid[1].set_ylabel(plotter.format_label(torch.VarType('\mathrm{Physical\ Size}', units='asec')))

grid[0].set_xlim([2, 10000])
grid[1].set_xlim([2, 10000])

grid[0].set_ylim([0.0, 1.0])
grid[1].set_ylim([0.0, 1.0])

xbins = np.logspace(0.3, 4.3, 22)
ybins = np.arange(0, 1.0, 0.04)

counts_0, xedges_0, yedges_0 = np.histogram2d(simulated_survey[:,9], phy_sizes, bins=(xbins, ybins))
counts_1, xedges_1, yedges_1 = np.histogram2d(cornish_survey[:, 9], cornish_phy_sizes, bins=(xbins, ybins))
...
grid[0].hist2d(simulated_survey[:,9], phy_sizes, bins=(xbins, ybins))
grid[0].pcolormesh(xedges_0, yedges_0, np.swapaxes(counts_0, 0, 1))

grid[1].hist2d(cornish_survey[:,9], cornish_phy_sizes, bins=(xbins, ybins))
